...
async def m006_add_order_customer_fields(db):
    ...
    await db.execute("ALTER TABLE orders.orders ADD COLUMN shipped BOOLEAN DEFAULT false;")
...
async def m007_add_settings_fields(db):
    ...
    await db.execute("ALTER TABLE orders.extension_settings ADD COLUMN message_order_received TEXT;")
    await db.execute("ALTER TABLE orders.extension_settings ADD COLUMN message_order_shipped TEXT;")
    await db.execute("ALTER TABLE orders.extension_settings ADD COLUMN business_name TEXT;")
    await db.execute("ALTER TABLE orders.extension_settings ADD COLUMN business_address TEXT;")


async def m008_add_order_weight(db):
    """
    Add optional weight field to orders.
    """
